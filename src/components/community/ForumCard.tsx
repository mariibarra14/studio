      ...
      <CardHeader className="p-0">
        <div className="relative aspect-video w-full bg-muted">
          {event?.imagenUrl ? (
            <Image
              src={event.imagenUrl}
              alt={event.nombre || 'Forum event image'}
              fill
  ...
  return (
    <Card className="h-full flex flex-col overflow-hidden transition-shadow duration-300 hover:shadow-xl group">
      {canManage ? (
        ...
        <Link href={`/community/${forum.id}`} className="block flex flex-col flex-grow">
          {CardBody}
        </Link>
      )}
      <CardFooter className="p-4 pt-0 mt-auto justify-between items-center text-sm text-muted-foreground">
